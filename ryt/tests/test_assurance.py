...
class ExecutionBudget(unittest.TestCase):
    def test_larger_agent_investigations_share_the_existing_whole_job_budget(self):
        ...
        # 85 minutes shared by eight sessions starts at 637 seconds/session,
        # while small reviews can still use the unchanged 30-minute session cap.
        self.assertEqual(session_budget(6100,8,1000),637)
        self.assertEqual(session_budget(6100,1,1000),1800)
        self.assertEqual(session_budget(1900,3,1000),300)
        with self.assertRaises(ValueError):session_budget(1010,1,1000)
        with self.assertRaises(ValueError):session_budget(6100,0,1000)
